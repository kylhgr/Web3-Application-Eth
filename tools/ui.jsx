  ...
  return (
    <Wrapper>
      ...
      {walletAddress && contracts.length > 0 ? (
				...
				Object.entries(
					groupBy(contracts, "chainId"))
					.map( ([groupChainId, groupedContracts]) => {
						const { name } = chainById(Number(groupChainId));
						const isActive = chainId === Number(groupChainId);

						return (
							<VStack>
								<h3 style={{ color: isActive ? "var(--fg-default)" : "var(--fg-dimmest)"}}>
									{name}
								</h3>
								{
								groupedContracts
...
function ContractUI({ contract: { name, address, abi }, onRemove, chainIsActive, chainId }) {
        ...
        >
          ...
          <button
            ...
            style={{
						...
						aria-label={chainIsActive ? null : `Switch to ${chainData.name} to run this function`}
        ...
        {stateMutability === "nonpayable" ? (
          <HStack style={{ padding: 4, gap: 4, alignItems: "center" }}>
            ...
            <HelpIndicator text="Gas is ETH you pay to execute a transaction" />
				...
				{
					!chainIsActive ? (
						<a style={{cursor: 'pointer'}} onClick={()=>switchToChain(chainId)}>Switch to {chainData.name}</a>
					) : 
					null
				}
      </VStack>
